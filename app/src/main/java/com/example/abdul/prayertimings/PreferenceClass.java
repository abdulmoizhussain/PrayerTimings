...
public class PreferenceClass extends PreferenceActivity implements OnSharedPreferenceChangeListener, Preference.OnPreferenceClickListener {
	...
	protected void onCreate (Bundle savedInstanceState) {
		...
		this.editor = sharedPreferences.edit(); editor.apply();
		...
		switch (key) {
			case "notification_switch":
				if (sharedPreferences.getBoolean (key, false)) {
					this.global.cancelNotifications (this);
					try {
						this.global.setNotifications(this);
			...
			case "silence_duration":
				...
				this.global.setToSilentMode(PreferenceClass.this);
		}
	}
	...
	private void checkAndSetSilenceSwitch () {
		...
		this.listPreference.setSummary(Integer.toString(this.sharedPreferences.getInt ("silence_duration", 10))+" minutes");
		for (int i=0; i<5; i++)
			this.preference[i].setSummary(Global.formatThisTime(PreferenceClass.this, loadLong(i+11)));
	}
	
	Dialog timePickerDialog () {
		return new TimePickerDialog(PreferenceClass.this, onTimePickerListener, hour, minute,
	...
	protected TimePickerDialog.OnTimeSetListener onTimePickerListener = new TimePickerDialog.OnTimeSetListener() {
		...
		public void onTimeSet (TimePicker view, int hourOfDay, int minuteOfDay) {
			minute = minuteOfDay; hour = hourOfDay;
			saveLong (id);
